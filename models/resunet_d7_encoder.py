...
class ResUNet_d7_encoder(HybridBlock):
    """
    This will be used for 256x256 image input, so the atrous convolutions should be determined by the depth
    """
    
    def __init__(self,_nfilters_init,  _NClasses,  verbose=True, psp_depth=3, _norm_type = 'BatchNorm', **kwards):
        HybridBlock.__init__(self,**kwards)
        
        self.model_name = "ResUNet_d7_encoder"
        ...
        with self.name_scope():
            ...
            self.pool5 = DownSample(nfilters, _norm_type = _norm_type)

            # Usually 1024
            nfilters = self.nfilters * 2**(5)
            if verbose:
                print ("depth:= {0}, nfilters: {1}".format(5,nfilters))
            self.Dn6 = ResNet_v2_unit(nfilters, _norm_type = _norm_type)
            self.pool6 = DownSample(nfilters, _norm_type = _norm_type)


            # Usually 2048
            nfilters = self.nfilters * 2**(6)
            if verbose:
                print ("depth:= {0}, nfilters: {1}".format(6,nfilters))
            self.Dn7 = ResNet_v2_unit(nfilters, _norm_type = _norm_type)

            
            # Same number of filters, with new definition  
            self.middle = PSP_Pooling(nfilters, _norm_type = _norm_type, depth = psp_depth)
    ...
    def hybrid_forward(self,F,_input):
        ...
        Dn6 = self.Dn6(pool5)
        pool6 = self.pool6(Dn6)

        Dn7 = self.Dn7(pool6)

        #pool7 = self.pool7(Dn7)
        #pool7 = F.UpSampling(pool7, scale=2, sample_type='nearest')
        #out = F.concat(Dn7,pool7,dim=1)
        #out = F.relu(out)

        middle = self.middle(Dn7)
        middle = F.relu(middle) # Restore channels 
