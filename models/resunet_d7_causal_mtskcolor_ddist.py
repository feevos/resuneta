...
from resuneta.models.resunet_d7_encoder import * 


class ResUNet_d7(HybridBlock):
    """
    ...
    def __init__(self,_nfilters_init,  _NClasses,  verbose=True,  _norm_type = 'BatchNorm', **kwards):
        ...
        with self.name_scope():
            ...
            self.encoder = ResUNet_d7_encoder(self.nfilters, self.NClasses,_norm_type=_norm_type)
            ...
            self.UpConv2 = ResNet_atrous_2_unit(nfilters,_norm_type=_norm_type)
    ...
    def hybrid_forward(self,F,_input):
        ...
        pool6 = self.encoder.pool6(Dn6)


        Dn7 = self.encoder.Dn7(pool6)
        middle = self.encoder.middle(Dn7)
        middle = F.relu(middle) # Activation of middle layers 
        
        
        UpComb1 = self.UpComb1(middle,Dn6)
        UpConv1 = self.UpConv1(UpComb1)
        
        UpComb2 = self.UpComb2(UpConv1,Dn5)
        UpConv2 = self.UpConv2(UpComb2)

        UpComb3 = self.UpComb3(UpConv2,Dn4)
        UpConv3 = self.UpConv3(UpComb3)
        
        UpComb4 = self.UpComb4(UpConv3,Dn3)
        UpConv4 = self.UpConv4(UpComb4)
        
        UpComb5 = self.UpComb5(UpConv4,Dn2)
        UpConv5 = self.UpConv5(UpComb5)
        
        UpComb6 = self.UpComb6(UpConv5,Dn1)
        UpConv6 = self.UpConv6(UpComb6)

         # second last layer 
        convl = F.concat(conv1,UpConv6)
